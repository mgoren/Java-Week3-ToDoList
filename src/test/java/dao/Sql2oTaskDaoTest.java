...
public class Sql2oTaskDaoTest {
    ...
    @Test
    public void getAll_returnsAllTasks_List() throws Exception {
        Task task = new Task("mow the lawn");
        taskDao.add(task);
    ...
    @Test
    public void update_updatesDescription() throws Exception {
        Task task = new Task("mow the lawn");
        taskDao.add(task);
        taskDao.update(task.getId(), "take a nap");
        assertEquals("take a nap", taskDao.findById(task.getId()).getDescription());
    }

    @Test
    public void deleteById_deletesTaskWithId() throws Exception {
        Task task = new Task("mow the lawn");
        taskDao.add(task);
